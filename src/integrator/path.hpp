...
struct single_path_t {
  ...
  color_t li(const Scene& scene, segment_t& s) const {
    ...
    segment.wi = sample.sampled - info.p;
    ...
    if (in_same_hemisphere(wi, segment.n) && !scene.occluded(info.ray(wi), d)) {
      ...
      out += segment.beta * (light->emit() * bxdf->f(il, ol)).scale(s);
    }
    ...
    segment.beta = beta * (r * (abs_dot(path.direction, info.n) / next.pdf));

    if (depth > 2 && terminate_ray(segment.beta)) {
      break;
    }
