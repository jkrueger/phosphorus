...
namespace bounds {
  ...
  */
  ...
  inline float8_t intersect_all<8>(
    ...
    auto min_x = select(gtez_x, bounds.max[0], bounds.min[0]);
    auto min_y = select(gtez_y, bounds.max[1], bounds.min[1]);
    auto min_z = select(gtez_z, bounds.max[2], bounds.min[2]);
    auto max_x = select(gtez_x, bounds.min[0], bounds.max[0]);
    auto max_y = select(gtez_y, bounds.min[1], bounds.max[1]);
    auto max_z = select(gtez_z, bounds.min[2], bounds.max[2]);
